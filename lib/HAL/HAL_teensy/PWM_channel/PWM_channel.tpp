...
template <typename T_PWM>
bool PWM_channel<T_PWM>::config(uint32_t frequency_hz_, uint32_t resolution_bit_)
{
    ...
    analogWrite(*pwm, pwm_to_bit_val(0));
    analogWriteResolution(resolution_bit);
...
template <typename T_PWM>
void PWM_channel<T_PWM>::set_pwm(double pwm_us_)
{
    if (pwm_us_ < 0.0)
    {
        pwm_us_ = 0.0;
    ...
    analogWriteResolution(resolution_bit);
    analogWrite(*pwm, pwm_to_bit_val(pwm_us_));
    analogWriteResolution(resolution_bit);
}
...
void PWM_channel<T_PWM>::set_duty(double duty_percent_)
{
    ...
    analogWriteResolution(resolution_bit);
    analogWrite(*pwm, (double)((1 << resolution_bit) - 1) * duty_percent_ * 0.01);
    analogWriteResolution(resolution_bit);
}
